...
class Query2<T: TableSchema, A, B>(val a: AbstractColumn<A, T, *>, val b: AbstractColumn<B, T, *>): Query<Pair<A, B>, T>(array(a, b)) {
    override fun where(op: Op): Query2<T, A, B> {
        return super.where(op) as Query2<T, A, B>
    }
}

class Query1<T: TableSchema, A>(val a: AbstractColumn<A, T, *>): Query<A, T>(array(a)) {
